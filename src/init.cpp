...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Pleskov.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Pleskovd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizepleskovamount=<n> " + _("Keep N Pleskov anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pleskov is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pleskov is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Pleskov version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Pleskov server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pleskov") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pleskov to complete") << "\n";
    ...
    nAnonymizePleskovAmount = GetArg("-anonymizepleskovamount", 0);
    if(nAnonymizePleskovAmount > 999999) nAnonymizePleskovAmount = 999999;
    if(nAnonymizePleskovAmount < 2) nAnonymizePleskovAmount = 2;
    ...
    LogPrintf("Anonymize Pleskov Amount %d\n", nAnonymizePleskovAmount);
